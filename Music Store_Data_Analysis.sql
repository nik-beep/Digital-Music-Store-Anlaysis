...
(
	...
	ROW_NUMBER() OVER(PARTITION BY customer.country ORDER BY customer.country,COUNT(invoice_line.quantity) DESC) AS RowNo 
    FROM invoice_line 
...
WITH Customter_with_country AS (
	    ...
	    ROW_NUMBER() OVER(PARTITION BY billing_country ORDER BY billing_country,SUM(total) DESC) AS RowNo 
		FROM invoice
		JOIN customer ON customer.customer_id = invoice.customer_id
